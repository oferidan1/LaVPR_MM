...
from vpr_models.crica_small.vision_transformer import vit_small, vit_base, vit_large, vit_giant2
import math
...
class CricaVPRNet(nn.Module):
    ...
    """
    def __init__(self, pretrained_foundation = False, foundation_model_path = None, d_model=384):
        super().__init__()
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=16, dim_feedforward=2048, activation="gelu", dropout=0.1, batch_first=False)
...
def get_cricavpr_small():
    model = CricaVPRNet()    
    model_path = 'trained_models/cricavpr_small.pth'
